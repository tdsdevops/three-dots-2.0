...
function updatePaletteTexture(tex, stops) {
  let arr;
  if (Array.isArray(stops) && stops.length > 0) {
    if (stops.length === 1) {
      arr = [stops[0], stops[0]];
...
class ExternalForce extends ShaderPass {
  constructor(simProps, common, mouse) {
...
class Output {
  constructor(common, mouse, paletteTex, bgVec4) {
    this.common = common;
    this.mouse = mouse;
    this.init(paletteTex, bgVec4);
  }
  init(paletteTex, bgVec4) {
    this.simulation = new Simulation({}, this.common, this.mouse);
    this.scene = new THREE.Scene();
    this.camera = new THREE.Camera();
...
class WebGLManager {
  constructor(props, paletteTex, bgVec4) {
    this.props = props;
...
export default function LiquidEther({
  ...
  // Initialize WebGLManager
  useEffect(() => {
    if (!mountRef.current) return;

    const paletteTex = makePaletteTexture(colors);
    paletteTexRef.current = paletteTex;
    const bgVec4 = new THREE.Vector4(0, 0, 0, 0); // fully transparent
    ...
    // ResizeObserver to handle canvas resizing performantly
  ...
  // Handle prop updates dynamically without tearing down the WebGL scene/context
  useEffect(() => {
    ...
    // Update Palette Texture in place if colors change
    if (paletteTexRef.current) {
      updatePaletteTexture(paletteTexRef.current, colors);
    }
